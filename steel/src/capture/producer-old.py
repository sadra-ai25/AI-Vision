        ...
        try:
            ...
            while process.poll() is None and (stop_event is None or not stop_event.is_set()):
                ...
                while len(buffer) >= frame_size:
                    ...
                    t0=time.time()
                    rabbitmq_client.publish(f"frame_queue_{camera_id}", pickle.dumps(compressed_frame))
                    dt=time.time()-t0
    ...
    try:
        while cap.isOpened() and (stop_event is None or not stop_event.is_set()):
            ...
            try:
                ...
                rabbitmq_client.publish(f"frame_queue_{processor_id}", pickle.dumps(compressed_frame))
                frame_count += 1
